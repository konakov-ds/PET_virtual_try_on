...
if __name__ == '__main__':
    ...
    for epoch in range(1, 2):
        ...
        for i, data in enumerate(dataset, start=epoch_iter):
            ...
            flow_out = warp_model(real_image.to('cpu'), clothes.to('cpu'))
            warped_cloth, last_flow, = flow_out
            warped_edge = F.grid_sample(edge.to('cpu'), last_flow.permute(0, 2, 3, 1),
                                        mode='bilinear', padding_mode='zeros')

            gen_inputs = torch.cat([real_image.to('cpu'), warped_cloth, warped_edge], 1)
